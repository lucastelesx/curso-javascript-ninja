...
console.log(`
  ${maskCPF(cleanCPF('049-214 3421-1'))}
  ${maskCPF(cleanCPF('210.458.522-05'))}
  ${maskCPF(cleanCPF('735 500 794 - 22'))}
  ${maskCPF(cleanCPF('101.123-131x32'))}  
`)
/*
...
var 
  fraseJ = "Os meses de janeiro, junho e julho começam com a letra j.",
  regexJ = /junho|julho/g,
  matchesJ = fraseJ.match(regexJ)
console.log(matchesJ);
/*
...
var 
  elementTest = "<div><section><blockquote>Texto <img /></blockquote></section></div>",
  elementRegex = /<\w+\>/g,
  matchesElements = elementTest.match(elementRegex)
console.log(matchesElements);
...
var 
  elementTestEmpty = "<div><ul><li></li><li></li><li><span></span></li></ul></div>",
  elementRegexEmpty = /<\w+\>\<\/\w+\>/g,
  matchesElementsEmpty = elementTestEmpty.match(elementRegexEmpty)
console.log(matchesElementsEmpty);
...
var 
  elementTestText = "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>",
  elementRegexText = /<(\w+)>([^<]+)\<\/\w+>/g,
  matchesElementsText = elementTestText.match(elementRegexText)
  console.log(
    elementTestText.replace(elementRegexText,
      `<$1>O texto dentro da tag "$1" é "$2"</vvvvvvvvvvvvvvvvvvvvvvv$1>\n`
    )
  )
})()